...
tests :: TestSuite
tests =
  suite "query param encoding" do
    suite "EncodeQueryParam" do
      suite "String" do
        test "encodes string without special character as is"
          $ do
              Assert.equal (Right $ Just "asdf") (encodeQueryParam "asdf")
        test "URL encodes string with special characters"
          $ do
              Assert.equal (Right $ Just "%C3%A4iti%20ja%20is%C3%A4") (encodeQueryParam "äiti ja isä")
      suite "Boolean" do
        test "encodes true as \"true\""
          $ do
              Assert.equal (Right $ Just "true") (encodeQueryParam true)
        test "encodes false as \"false\""
          $ do
              Assert.equal (Right $ Just "false") (encodeQueryParam false)
    suite "EncodeQueryParamMulti" do
      suite "Object (Array String)" do
        test "URL encodes strings with special characters"
          $ do
        ...
        test "encodes array of values as separate key value pairs"
          $ do
