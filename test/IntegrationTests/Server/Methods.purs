...
tests = do
  ...
  suite "Methods" do
    ...
    suite "POST" do
      ...
      test "POST succeeds with empty body route"
        $ do
            let
              spec =
                Spec ::
    ...
    suite "HEAD" do
      ...
      test "HEAD is accepted where GET route is defined"
        $ do
            ...
            withRoutes spec handlers do
              res <- head "/foo"
              respMatches { status: 200, body: "" } res
      test "user-specified HEAD route overrides default GET HEAD route"
        $ do
    ...
    suite "DELETE" do
      ...
      test "DELETE succeeds with body"
        $ do
            let
              spec =
                Spec ::
                  _ { foo ::
                      DELETE "/foo" { body :: { id :: String }
                      , response :: String
                      }
                  }
