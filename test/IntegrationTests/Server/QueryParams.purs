...
tests = do
  ...
  suite "Query parameter requests" do
    suite "keys (e.g. foo=<myFoo>)" do
      test "GET /search?limit=3 succeeds with valid int"
        $ do
            let
              spec =
                Spec ::
      ...
      test "GET /search?limit=3.1 fails with invalid integer"
        $ do
            let
              spec =
                Spec ::
      ...
      test "GET /search?limit=asdf fails with non-integer"
        $ do
            let
              spec =
                Spec ::
            ...
            withRoutes spec handlers do
              ...
              respMatches { status: 400, body: "" } res
      test "GET /search fails with missing required limit key"
        $ do
            let
              spec =
                Spec ::
            ...
            withRoutes spec handlers do
              res <- get "/search"
              respMatches { status: 400, body: "" } res
      test "GET /search?limit=asdf succeeds with provided optional limit key"
        $ do
            let
              spec =
                Spec ::
      ...
      test "GET /search?limit=asdf succeeds with missing optional limit key"
        $ do
            let
              spec =
                Spec ::
      ...
      test "POST /profile?id=3&foo=asdf succeeds"
        $ do
            let
              spec =
                Spec ::
                  _ { profile ::
                      POST "/profile?id=<id>&foo=<foo>"
      ...
      test "POST /profile?id=3&foo=asdf succeeds when id and foo are given in another order"
        $ do
            let
              spec =
                Spec ::
            ...
            withRoutes spec handlers do
              ...
              respMatches { status: 200, body: "Saved profile" } res
      test "POST /profile?id=3.0&foo=asdf fails"
        $ do
            let
              spec =
                Spec ::
            ...
            withRoutes spec handlers do
              ...
              respMatches { status: 400, body: "" } res
      test "GET /search?query=asdf succeeds with some String"
        $ do
            let
              spec =
                Spec ::
      ...
      test "GET /search?query= succeeds with empty String"
        $ do
            let
              spec =
                Spec ::
      ...
      test "GET /search?query succeeds with missing String"
        $ do
            let
              spec =
                Spec ::
    ...
    suite "multi-match (e.g. foo=<myFoo>)" do
      test "GET /search?<..all> multimatch grabs all query parameters"
        $ do
            let
              spec =
                Spec ::
    ...
    suite "other checks" do
      test "GET /search?a=asdf&b=asdf&c=asdf ignores extra parameters (a and c)"
        $ do
            let
              spec =
                Spec ::
            ...
            withRoutes spec handlers do
              ...
              respMatches { status: 200, body: "Search result" } res
      test "GET /search?a=foo1&a=foo2 fails if given two duplicate params where one was specified"
        $ do
            let
              spec =
                Spec ::
