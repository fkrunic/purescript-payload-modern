...
getUser req = do
  if StringUtils.endsWith "username" (HTTP.requestURL req) then
    pure (Right (User { id: 1, name: "John User" }))
  else
    pure (Left (Forward "Not a user"))
...
tests = do
  ...
  suite "Guards" do
    ...
    test "routes to guarded handler if guard is passed (secret is provied)"
      $ do
    ...
    test "fails with 404 if guard is failed (no secret provided)"
      $ do
          withServer spec api do
            res <- get "/admin"
            Assert.equal 404 res.status
    test "routes to guarded handler if guard is passed (username is provided)"
      $ do
          withServer spec api do
            res <- get "/user?username"
            respMatches { status: 200, body: "User page" } res
    test "fails with 404 if guard is not passed (no username is provided)"
      $ do
          withServer spec api do
            res <- get "/user"
            Assert.equal 404 res.status
    test "unguarded call succeeds"
      $ do
          withServer spec api do
            res <- get "/"
            respMatches { status: 200, body: "Unauthenticated page" } res
    test "top-level guard: request fails if guard fails"
      $ do
          let
            spec_ =
              Spec ::
          ...
          withServer spec_ { guards: { adminUser: getAdminUser }, handlers: { adminIndex } } do
            res <- get "/admin"
            Assert.equal 404 res.status
    test "top-level guard: request succeeds if guard succeeds"
      $ do
          let
            spec_ =
              Spec ::
    ...
    test "parent routes guard: request fails if guard fails"
      $ do
          let
            spec_ =
              Spec ::
                ...
                , routes ::
                    { all ::
                        Routes "/all" { guards :: Guards ("adminUser" : Nil)
                        , adminIndex ::
                            GET "/admin" { response :: String
                            }
                        }
          ...
          withServer spec_ { guards: { adminUser: getAdminUser }, handlers: { all: { adminIndex } } } do
            res <- get "/admin"
            Assert.equal 404 res.status
    test "parent routes guard: request succeeds if guard succeeds"
      $ do
          let
            spec_ =
              Spec ::
          ...
          withServer spec_ { guards: { adminUser: getAdminUser }, handlers: { all: { adminIndex } } } do
            ...
            Assert.equal 200 res.status
    test "endpoint guard: request fails if guard fails"
      $ do
          let
            spec_ =
              Spec ::
                ...
                , routes ::
                    { adminIndex ::
                        GET "/admin" { guards :: Guards ("adminUser" : Nil)
                        , response :: String
                        }
                    }
                }
          withServer spec_ { guards: { adminUser: getAdminUser }, handlers: { adminIndex } } do
            res <- get "/admin"
            Assert.equal 404 res.status
    test "endpoint guard: request succeeds if guard succeeds"
      $ do
          let
            spec_ =
              Spec ::
