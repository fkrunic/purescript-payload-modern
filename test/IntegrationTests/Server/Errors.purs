...
tests = do
  ...
  suite "Errors" do
    test "returning Either String returns a String as an internal error"
      $ do
    ...
    test "returning Either String (Response String) returns a String as an internal error"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
          ...
          withRoutes spec handlers do
            res <- get "/foo"
            respMatches { status: 500, body: "Error" } res
    test "returning Either Array returns JSON as an internal error"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
          ...
          withRoutes spec handlers do
            res <- get "/foo"
            respMatches { status: 500, body: "Error" } res
    test "returning Either Array (Response String) returns JSON as an internal error"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
          ...
          withRoutes spec handlers do
            res <- get "/foo"
            respMatches { status: 500, body: "Error" } res
    test "returning Either (Response Record) String returns JSON"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
          let
            handlers =
    ...
    test "returning Either (Response Record) (Response String) returns JSON"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
          let
            handlers =
          ...
          withRoutes spec handlers do
            res <- get "/foo"
            Assert.equal { status: 400, body: "{\"id\": 1}", headers: Map.empty } res
    test "to return an arbitrary error response, return Either ServerError"
      $ do
          let spec = Spec :: _ { foo :: GET "/foo" { response :: String } }
